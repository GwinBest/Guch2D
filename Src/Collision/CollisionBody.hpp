...
namespace Guch2D
{
    ...
    public:
        ...
        explicit CollisionBody(const Vect& position) : _position(position)
        {
        }

        explicit CollisionBody(const std::shared_ptr<Collider>& collider) : _collider(collider)
        {
        }

        CollisionBody(const Vect& position,
